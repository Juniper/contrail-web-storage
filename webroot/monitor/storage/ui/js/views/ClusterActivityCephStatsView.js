...
define(['underscore', 'contrail-view', 'contrail-list-model'], function(_, ContrailView, ContrailListModel) {
    ...
    function getClusterActivityStatsViewConfig(){
        return {
            elementId: cowu.formatElementId([swl.CLUSTER_DISK_ACTIVITY_THRPT_STATS_ID, '-section']),
            view: "SectionView",
            viewConfig: {
                rows: [
                    {
                        columns: [
                            {
                                elementId: swl.CLUSTER_DISK_ACTIVITY_THRPT_STATS_TAB_ID,
                                view: "TabsView",
                                viewConfig: {
                                    ...
                                    activate: function (e, ui) {
                                        ...
                                        if (selTab == swl.TITLE_CLUSTER_CEPH_THROUGHPUT) {
                                            $('#' + swl.CLUSTER_CEPH_ACTIVITY_THRPT_CHART_ID).find('svg').trigger('refresh');
                                        } else if (selTab == swl.TITLE_CLUSTER_CEPH_LATENCY) {
                                            $('#' + swl.CLUSTER_CEPH_ACTIVITY_CEPH_LATENCY_CHART_ID).find('svg').trigger('refresh');
                                        }
                                    },
                                    tabs: [
                                        {
                                            elementId: swl.CLUSTER_CEPH_ACTIVITY_THRPT_CHART_ID,
                                        ...
                                        {
                                            elementId: swl.CLUSTER_CEPH_ACTIVITY_CEPH_LATENCY_CHART_ID,
